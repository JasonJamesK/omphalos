...
- **Architecture**: Prefer frontend-only changes — `Omphalos.Domain`/`Repository`/`Services` shouldn't need new dependencies for markdown itself (plain text storage in existing string fields); may still need a migration if any target column has a length limit too small for markdown-formatted long text.
