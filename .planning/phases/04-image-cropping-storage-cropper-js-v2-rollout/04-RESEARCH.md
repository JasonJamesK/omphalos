...
1. **How should new crop bytes travel on the WRITE side for session-scoped Character/Location, given `CharacterDto`/`LocationDto` are used bidirectionally in `SessionDto`/`UpsertSessionRequest`?** — RESOLVED: Option B, implemented in 04-01-PLAN.md.
   ...
   - Resolution: **Option B**, adopted for this phase. It requires no new endpoint routes for the write direction, needs no change to `dispatchWithPersist`'s architecture, and solves the "character not yet persisted server-side" sequencing problem for free (image bytes ride along in the very same upsert call that first creates the row) — Cropper.js v2 wiring and the backend migration are already substantial scope for one MVP-mode phase; Option A's stricter read/write separation can be a follow-up hardening pass once the vertical slice ships. This was surfaced to the user/planner explicitly rather than silently decided — 04-01-PLAN.md implements Option B via a 4-rule write contract (`<image_write_contract>`), and downstream plans (04-04/05/06) build on it.

2. **Exact `<cropper-image>` ready/load event name and timing in cropperjs 2.1.1** — RESOLVED: verification deferred to implementation, tracked as an explicit first sub-task in 04-02-PLAN.md (not a blind guess baked into the plan).
   ...
   - Resolution: 04-02-PLAN.md's Task 2 requires inspecting the installed `cropperjs@2.1.1` package (TypeScript definitions/source in `node_modules/cropperjs`) as the first sub-task of implementation, before writing the full `ImageCropModal` — a 5-minute check that removes real risk from the highest-flagged-risk part of this phase, rather than leaving it as an unverified assumption.
