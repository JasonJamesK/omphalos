...
{
    public static IEndpointRouteBuilder MapCharacterImageEndpoints(this IEndpointRouteBuilder app)
    {
        var group = app.MapGroup("/api/sessions").RequireAuthorization();

        group.MapGet("/{sessionId}/characters/{id}/portrait/{variant}", async (
            string sessionId, string id, string variant, ClaimsPrincipal user, IImageService images, HttpContext http, CancellationToken ct) =>
        {
            ...
            var bytes = await images.GetSessionCharacterImageAsync(sessionId, id, userId, variant == "cropped", ct);
